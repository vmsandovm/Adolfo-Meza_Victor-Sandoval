...
module MultipleModeRegister_piso_lsb_circ_tb;
	...
	MultipleModeRegister #( .DW(DW), .MODE(PISO_LSB_CIRC_MODE) ) dut (
		.clk        (clk),
...
// Estimulos: l_s = 1 carga en paralelo, l_s = 0 rota. La salida serie es out[0]
	initial begin
	clk        =  0;
	sync_reset =  1;
	enb        =  1;
...
// PISO LSB first circular
	...
	if (errores == 0)
		$display("\n=== PISO_LSB_CIRC_MODE: PASS (0 errores) ===\n");
	else
		$display("\n=== PISO_LSB_CIRC_MODE: FAIL (%0d errores) ===\n", errores);
